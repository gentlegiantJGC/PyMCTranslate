...
class App(QtWidgets.QApplication):
    ...
    def __init__(self):
        ...
        self.input_versions = self.mappings.version_numbers(
            self.mapping_platforms[self.input_platform.currentIndex()]
        )
        ...
        self.input_sub_versions = list(
            self.mappings.get(
                self.mapping_platforms[self.input_platform.currentIndex()],
                self.input_versions[self.input_version.currentIndex()],
            )._subversions.keys()
        )  # I need to add an actual API entry for this
        self.input_sub_version.addItems(self.input_sub_versions)
        self.input_sub_version.setCurrentIndex(0)
        self.input_sub_version.currentIndexChanged.connect(
        ...
        self.output_versions = self.mappings.version_numbers(
            self.mapping_platforms[self.output_platform.currentIndex()]
        )
        ...
        self.output_sub_versions = list(
            self.mappings.get(
                self.mapping_platforms[self.output_platform.currentIndex()],
                self.output_versions[self.output_version.currentIndex()],
            )._subversions.keys()
        )  # I need to add an actual API entry for this
        self.output_sub_version.addItems(self.output_sub_versions)
        self.output_sub_version.setCurrentIndex(0)
        self.output_sub_version.currentIndexChanged.connect(
