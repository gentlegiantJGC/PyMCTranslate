...
def get_version(platform: str, version: str) -> Version:
    assert (
        platform in translations.platforms()
    ), f"Platform {platform} is not valid. Must be one of {translations.platforms()}"
    version = tuple(int(i) for i in version.split("."))
    return translations.get_version(platform, version)


print(
    'Example command: "java 1.12.2 false bedrock 1.13.0 false minecraft:double_stone_slab[block_data=0]"'
)
print(
    'Example command: "java 1.12.2 true bedrock 1.13.0 false minecraft:trapdoor[facing=south,half=top,open=false]"'
)
