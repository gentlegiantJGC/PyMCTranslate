    ...
    for platform_name in block_mappings.platforms:
        for version_number in block_mappings.version_numbers(platform_name):
            ...
            if not version.block_format == "pseudo-numerical":
                print(
                    f"skipping {platform_name} {version_number}. Not pseudo-numerical format"
                )
                continue
            ...
            for namespace_str in input_version.namespaces:
                for block_name in input_version.block_names(namespace_str):
                    block_specification = input_version.get_specification(
                        "block", namespace_str, block_name
                    )
                    ...
                    for spec_ in itertools.product(*values):
                        ...
                        try:
                            (
                                ...
                                extra_output,
                                extra_needed,
                            ) = input_version.to_universal(
                                None, Block(namespace_str, block_name, spec)
                            )
                        except:
                            output = extra_output = None
                            print(
                                {
                                    "block_name": f"{namespace_str}:{block_name}",
                                    "properties": spec,
                                }
                            )
                            continue
